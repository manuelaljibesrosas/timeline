      ...
      <main className="flex flex-col bg-[#121314]">
        <div className="flex flex-col justify-center h-[100px] px-[32px] border-b-[1px] border-solid border-[#1b1f20]">
          <div className="flex flex-col gap-[4px]">
            ...
            <h1 className="text-[28px] leading-[1.14] font-bold text-[#fff]">{"Dispatch Center"}</h1>
          </div>
        </div>
        ...
        <Spring to={{ visibleTimeStart, visibleTimeEnd }}>
          {(value: any) => (
            <AnimatedTimeline
                ...
                setItems((items) => {
                  ...
                  itemsInGroup.forEach((item) => {
                    ...
                    if (
                      item.start_time > selectedItem.end_time &&
                      (nextItem === null ||
                        nextItem.start_time > item.start_time)
                    )
                      nextItem = item;
                  });
              ...
              buffer={1}
              onTimeChange={() => {}}
              groupRenderer={(props) => <GroupItem {...props} />}
            >
              <TimelineHeaders>
                <SidebarHeader>
                  {({ getRootProps }) => {
              ...
              <TimelineMarkers>
                <TodayMarker date={Date.now()}>
                  {({ styles: { backgroundColor, width, ...styles } }) => (
                    <div
                      style={styles}
                      className="z-[999] w-[1px] bg-[#0465ff]"
                    >
